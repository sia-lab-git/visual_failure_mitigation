from xpc3 import *
from xpc3_helper_KSFO import *
from xpc3_helper_SK_KSFO import *
from nnet import *
...
def getClassifierOutput(model, img):
...
def get_simulator_and_model(start_params, classifier_weights):
    ...
    client.sendDREF("sim/time/zulu_time_sec", TIME_OF_DAY*3600+8*3600) # Adding 8 to convert to UTC, specific to runway location 
    ...
    classifier = torch.load(classifier_weights, map_location=device)
    classifier.eval()

    return client, network, classifier
...
def rollout(client, network, num_steps=500, dt=0.05, ctrl_every=1, sim_dynamics='dubins', ideal=0, image_getter = getCurrentImage, classifier=None, use_fallback=False):
    ...
    for i in range(num_steps):
        ...
        if i % ctrl_every == 0:
            ...
            if use_fallback:
                classifier_output = getClassifierOutput(classifier, orig)
                print('classifier output:', classifier_output)
                time.sleep(0.01)

                if classifier_output == 1:
                    while classifier_output != 0:

                        phi_deg = -0.74 * cte - 0.44 * he
                        velocity -= 0.01
                        velocity = max(velocity, 0)
                        print('velocity:', velocity)
                        velocities.append(velocity)
                        cte, he, dtp = dynamics(cte, dtp, he, phi_deg, dt, velocity)
                        xs_record.append(cte)
                        he_record.append(he)
                        ys_record.append(dtp)
                        print('cte:', cte, 'dtp:', dtp, 'he:', he)
        ...
        if abs(cte) > 25.0:              # Safety limit different for reach runway
            print('System entered the unsafe state')
            result = 0
            episode_steps = i

        time.sleep(0.01)
    return xs, ys, thetas, pred_xs, pred_thetas, control, orig_images, downsampled_images, result, episode_steps, orig_images_pred, downsampled_images_pred, xs_record, he_record, ys_record, velocities
...
def plot_trajectory(x, y, use_fallback):
